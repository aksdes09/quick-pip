...
(function pageAutoPiP() {
    ...
    function mutationTouchesVideo(mutation) {
    ...
    function disconnectVideoObserver(reason) {
        if (!state.observer) return;
        try { state.observer.disconnect(); } catch (_) { }
        state.observer = null;
        debugLog('page_observer_disconnected', { reason });
    }

    function connectVideoObserver(reason) {
        if (state.observer) return;
        try {
            state.observer = new MutationObserver((mutations) => {
                if (mutations.some(mutationTouchesVideo)) scheduleRefresh();
            });
            state.observer.observe(document.documentElement || document.body, {
                childList: true,
                subtree: true
            });
            debugLog('page_observer_connected', { reason });
        } catch (_) { }
    }
    ...
    if (state.listenerVersion !== LISTENER_VERSION) {
        ...
        document.addEventListener('visibilitychange', () => {
            ...
            if (document.visibilityState === 'hidden') {
                ...
                disconnectVideoObserver('tab_hidden');
            } else {
        ...
        if (document.visibilityState === 'visible') {
            connectVideoObserver('initial_setup');
        }
