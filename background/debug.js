(function initDebug(root) {
  ...
  // In-memory cache of the debug-enabled flag so debugLog() - called on
  // almost every internal event - doesn't have to hit chrome.storage.local
  // just to learn logging is off. That storage read also keeps the MV3
  // service worker from idling out, so skipping it when we already know the
  // answer lets the worker suspend normally. null = not primed yet for this
  // service worker lifetime.
  let debugEnabledCache = null;

  function setDebugEnabledCache(value) {
    debugEnabledCache = value === true;
  }

  function refreshDebugEnabledCache() {
    try {
      if (typeof chrome === 'undefined' || !chrome.storage || !chrome.storage.local) return;
      chrome.storage.local.get([STORAGE_KEYS.autoPipDebugEnabled], (data) => {
        debugEnabledCache = !!(data && data[STORAGE_KEYS.autoPipDebugEnabled] === true);
      });
    } catch (_) { }
  }

  function writeDebugEntry(source, event, details) {
    try {
      chrome.storage.local.get([
        ...
        const text = typeof (data && data[STORAGE_KEYS.autoPipDebugText]) === 'string'
  ...
  function debugLog(source, event, details = {}) {
    try {
      ...
      if (debugEnabledCache === null) {
        // Not primed yet this service-worker lifetime (e.g. the very first
        // call right after a wake-up): fall back to a one-time read, which
        // also primes the cache so every later call in this lifetime is
        // synchronous.
        chrome.storage.local.get([STORAGE_KEYS.autoPipDebugEnabled], (data) => {
          debugEnabledCache = !!(data && data[STORAGE_KEYS.autoPipDebugEnabled] === true);
          if (debugEnabledCache) writeDebugEntry(source, event, details);
        });
        return;
      }

      if (debugEnabledCache !== true) return;
      writeDebugEntry(source, event, details);
    } catch (_) { }
  }
  ...
  AutoPip.debug = {
    ...
    refreshDebugEnabledCache,
    setDebugEnabledCache
  };
